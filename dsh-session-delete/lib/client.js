...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    // ── Settings tab: user-installed plugins (plugin manager) ─────────────────
    ...
    function CustomPluginsTab() {
      ...
      const load = () => {
        fetch('/dsh-session-delete/plugins')
      ...
      const toggle = async (plugin) => {
        ...
        try {
          const response = await fetch('/dsh-session-delete/plugins/toggle', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
      ...
      return react.createElement('ul', { style: tabListStyle }, state.plugins.map((plugin) => {
        ...
        const isSelf = plugin.entryId === 'session-delete-menu'
        const toggleStyle = enabled ? tabToggleOnStyle : tabToggleStyle
        const toggleLabel = enabled ? '已启用' : '已禁用'
        ...
        return react.createElement('li', { key: plugin.id, style: tabCardStyle }, [
          ...
          react.createElement('span', {
            style: { flex: 'none', fontSize: '12px', color: enabled ? 'var(--dsw-alias-state-success-primary,#30a46c)' : 'var(--dsw-alias-label-tertiary,#888)' }
          }, toggleLabel),
