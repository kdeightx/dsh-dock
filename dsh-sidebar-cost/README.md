...
- **数据自研**：`/ds-balance` 与 `/ds-activity` 路由完全自研（v0.1.1 起），
  零第三方运行时依赖，数据引擎内联自 [dsh-cost-crystal](https://github.com/xxvk/dsh-cost-crystal)
  （© xxvk, Apache-2.0，见下方来源声明）。
...
依赖：无（数据路由自研，零第三方运行时依赖）。

## 结构

```
lib/index.js    Host 端：注册 /ds-balance、/ds-activity 路由（含 2s 幂等自愈）
