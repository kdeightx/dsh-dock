// dsh-system-shutdown browser half: the sidebar-foot 「关闭 DSH」 row, styled
// like the settings trigger, with a DSH-styled confirm modal.
window.__ModuleLoader__.load({
  id: 'dsh-system-shutdown',
  factory: (require) => {
    var module = { exports: {} }
      ...
      if (confirmBtn !== null) {
        confirmBtn.addEventListener('click', async () => {
          ...
          try {
            ...
            console.error('[dsh-system-shutdown]', error)
          }
        })
      }
    ...
    function ShutdownButton({ wide }) {
      ...
      const click = () => {
        showModal({
          ...
          cancelLabel: '取消',
          onConfirm: async () => {
            fetch('/dsh-system-shutdown/action', { method: 'POST' }).catch(() => { /* process is going away */ })
            return true
          }
        })
      }
      ...
      return react.createElement('button', {
        ...
        wide && react.createElement('span', null, '关闭 DSH')
      ])
    }
